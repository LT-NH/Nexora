...
const changelogData: ChangelogEntry[] = [
  {
    ...
    date: '2026年9月19日',
    changes: [
      '修复深浅色主题「要一个页面一个页面单独切换」：排查发现根因不是单点故障，而是**三个互抢的写入方**都在直接改 <html> 上的 dark class，谁最后执行谁生效 —— 于是不同页面、甚至同一屏内不同入口之间表现不一致',
      '写入方一：主题 hook 原先只是内部持有 useState 的普通 hook，于是顶栏、命令面板、**每一个 ECharts 实例**各持一份独立状态、互不同步。表现为：顶栏切到深色后图表不变色；从命令面板切换时，顶栏图标不更新（因为它基于自己那份陈旧状态）',
      '写入方二：品牌白标逻辑又写一次 class，且规则与主题 hook **相互矛盾** —— 用户选「跟随系统」且系统为深色时会按品牌设置覆盖成浅色；品牌默认深色时又会加 dark，而切换器仍显示「浅色模式」',
      '写入方三：营销首页用 MutationObserver 把 dark class 强行按住（固定浅色设计），并在卸载时用**挂载时捕获的旧值**恢复 —— 期间用户若切过主题就会还原错',
      '修复方式：新增 ThemeProvider 作为**唯一事实来源与唯一 class 写入方**，其余模块只表达意图。优先级明确为：用户显式选择 > 品牌默认（仅当用户从未选择过）> 系统偏好；营销首页的固定浅色改为向主题层声明，不再篡改用户选择，离开后自动恢复',
      '主题 hook 保留原 import 路径（改为转发层），因此顶栏、命令面板、图表 hook 等既有调用点**零改动**',
      '新增 6 项回归守卫：跨消费者同步（面板切换 → 顶栏同步）、切换器下一态一致、主题跨页面保持、刷新持久化、「跟随系统」真正尊重系统深色且不被品牌覆盖、营销首页固定浅色但不篡改用户选择',
      '并验证了守卫的有效性：临时还原旧实现后，用例「跨消费者同步」与「跟随系统不被品牌覆盖」确实失败 —— 证明测试抓的正是这两个回归，而非摆设',
    ],
  },
  {
    version: 'v5.10',
    date: '2026年9月19日',
    changes: [
      '清理数据库孤儿数据并**开启 SQLite 外键强制**（v5.9 中记录为待办的事项已完成）：共清理 15 行 —— 10 处外键违规行（products 4 / customers 3 / orders 3）加上它们级联的 5 条 order_items。这些数据全部是淘宝/拼多多适配器联调时的测试残留（TB-9001 / PDD-… / 「淘宝测试商品」），所属的 4 个测试工作空间早已删除，但因当时外键未强制，删父行没有级联，子行就此残留',
      '清理过程按「先备份、再空跑、后落刀」执行：用 SQLite backup API 生成完整备份（WAL 模式下直接拷 .db 文件可能漏掉尚未 checkpoint 的数据），随后空跑计算删除闭包并做反向校验（确认没有其它表引用待删订单，结果为 0 遗漏），最后在单一事务内按「子行先删」的顺序提交。执行后 PRAGMA foreign_key_check 返回 0',
      '开启 PRAGMA foreign_keys=ON。这不是「顺手打开」：清理之前开启会让那 10 处脏数据直接导致后续写入报错。现在数据库会真正阻止悬空引用与残留子行',
      '让测试也跑在外键强制下（conftest 与生产共用同一份 PRAGMA 配方）。这一步立刻抓出 6 个用例在用凭空生成的 uuid 引用不存在的 order / user —— 生产环境里 user_id 来自鉴权主体、order_id 来自真实订单，这类悬空引用不可能出现，属测试夹具缺陷；已统一改为「先建真实行再引用」。这正是让测试跑在真实约束下的价值：以前它们只在断言一个生产里不存在的状态',
      '新增外键强制守卫测试（级联删除生效、悬空插入被拒），防止该 PRAGMA 日后被误删',
    ],
  },
  {
    version: 'v5.9',
  ...
  },
  {
    ...
    date: '2026年9月17日',
    changes: [
      '修复经营健康引擎头部两个按钮「点击无反应」：实测（elementFromPoint 命中检测）按钮并未被遮挡、点击事件也正常触发 —— 真正的问题是**点了之后没有任何视口内可见的反馈**',
      '「评分方法」弹层从页面很下方（雷达卡之后）改为**紧贴按钮下方弹出**（320px 浮层 + 点击外部关闭 + aria-expanded），点开即刻可见，不再需要滚动到页面底部找变化',
      '「重新体检」增加明确的状态反馈：按钮进入高亮禁用态（aria-busy）+ 按钮旁出现「正在重新体检，约需 10~30 秒」状态胶囊（role=status）。该端点每次都真实重算并调用千问，耗时 10~35 秒 —— 原先只有 16px 图标转圈，期间界面无任何变化，必然被当成没反应',
      '体检完成后新增 toast 反馈（「体检完成，数据已更新」）；失败时不再清空整卡数据（原来失败会把卡片打回一条加载文案），改为保留旧数据 + 错误 toast；页面首载不再弹完成提示（只有手动重新体检才提示）',
      '新增交互回归守卫（radar-side-cards.spec.ts 第 3 项）：断言弹层紧贴按钮下方（间距 <24px）且完整落在视口内、含六个维度口径、点击外部关闭；重新体检后必须有完成/失败反馈',
    ],
  },
  {
    version: 'v5.6',
